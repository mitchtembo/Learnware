...
export class GeminiService {
  private static API_KEY = import.meta.env.VITE_GEMINI_API_KEY || 'AIzaSyCub1GCnJQnHv6PoSofZ2t6UNDhPKEPWJM';
  ...
  static async generateCourseContent(
    ...
    try {
      ...
      return {
        ...
        recommendedReadings: sections[4].split('\n')
          .filter(item => item.trim())
  ...
  static async generateStudyMaterial(
    ...
    try {
      ...
      return {
        ...
        practiceQuestions: sections[3].split('\n')
          .filter(item => item.trim())
          .map(qa => {
